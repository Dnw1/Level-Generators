...
public class ChunkManager : MonoBehaviour
{
    ...
    void Start()
    {
		...
		Vector3 eersteChunckPos = m_chunkClones [0].transform.position;
		
		for (int k = 0; k < m_chunkClones.Count; k++) 
		{
			m_chunkClones[k].transform.position = new Vector3(eersteChunckPos.x+getChunkWidth(m_chunkClones[k])*k,0);
		}
    }
